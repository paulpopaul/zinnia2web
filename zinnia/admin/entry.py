...
class EntryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_('Atributos'), {
            'fields': ('edad','categoriass','nacionalidad', 'tez', 'altura',
                       'peso',('busto', 'cintura','cadera'),
                       'ojos', 'cabello', 'contextura',
                       ('busto2', 'cola') ,'depilacion',
                       ('idiomauno', 'idiomados')),
            'classes': ('collapse', 'collapse-closed')}),
        (_('Atención'), {
            'fields': ('atiendeuno', 'atiendedos', 'atiendetres', 'atiendecuatro'),
            'classes': ('collapse', 'collapse-closed')}),

        (_('Horario'), {
            'fields': ('dialunes', 'diamartes', 'diamiercoles', 'diajueves','diaviernes','diasabado','diadomingo'),
            'classes': ('collapse', 'collapse-closed')}),

        (_('Servicios'), {
            'fields': ('atencion_pareja', 'atencion_mujeres', 'atencion_discapaci', 'servicio_griego', 'frances_natural',
                       'sexo_oral', 'lesbico', 'garganta_profunda', 'duales_trios', 'americanaa', 'americana_corporal',
                       'masajes','baile_erotico', 'besos_novia', 'juguetes_eroticos', 'cuadro_plastico', 'despedida_soltero',
                       'viajess', 'sadismo', 'servicio_adicional_uno','servicio_adicional_dos', 'servicio_adicional_tres',
                       'servicio_adicional_cuatro' ,'servicio_adicional_cinco', 'servicio_adicional_seis',
                       'servicio_adicional_siete', 'servicio_adicional_ocho'),
            'classes': ('collapse', 'collapse-closed')}),

        (_('Imágenes'), {
            'fields': ('modeloretoque',('image','image2'),'imagal1', 'imagal2', 'imagal3', 'imagal4',
                       'imagal5', 'imagal6', 'imagal7', 'imagal8',
                       'imagal9','imagal10', 'imagal11', 'imagal12',
                       'imagal13', 'imagal14', 'imagal15', 'imagal16',
                       'imagal17', 'imagal18',),
            'classes': ('collapse', 'collapse-closed')}),
        (_('Administración'), {
            'fields': (('rut','nombreadmin'),
                       ('apellidouno', 'apellidodos'),
                       ('concelular', 'conemail'),
                       'certificadomedico', 'examenaldia',
                       'conrecordatorio', 'convalor', 'mesesatra'),
            'classes': ('collapse', 'collapse-closed')}),
        (_('Publication'), {
        ...
        (_('NO TOCAR'), {
            'fields': ('login_required', 'password','comment_enabled', 'pingback_enabled',
                       'trackback_enabled','sites','lead','content_template',
                       'detail_template','featured', 'excerpt', 'authors', 'related','tags'),
            'classes': ('collapse', 'collapse-closed')}),

        (None, {'fields': ('categories','ciudadheader', 'slug')}))

    list_filter = (CategoryListFilter, AuthorListFilter,
                   'publication_date', 'status')
    list_display = ('get_title', 'apellidouno',  'get_categories', 'ciudadheader','categoriass', 'get_is_visible', 'get_authors',
                    'publication_date','start_publication',
                    'end_publication')
    ...
    def get_title(self, entry):
        ...
        return title
    get_title.short_description = _('nombre')
    ...
    def get_categories(self, entry):
        ...
        try:
            ...
            return ', '.join([conditional_escape(category.title)
                              for category in entry.categories.all()])
    get_categories.short_description = _('Ciudad')
